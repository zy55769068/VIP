...
# 检查 config 文件是否存在
if [ ! -f "$DATA_PATH/config/config.sh" ]; then
    docker cp $CONTAINER_NAME:/ql/sample/config.sample.sh $DATA_PATH/config.sh
    if [ $? -ne 0 ] ; then
...
if [ "$port" = "2" ]; then
    ...
    log "6.1.用户名和密码访问面板设置，请登录 ip:$JD_PORT"
    echo -e "\n"
fi

# 防止 CPU 占用过高导致死机
echo -e "-------- 机器累了，休息 20s，趁机去操作一下吧 --------"
echo -e "-------- 新版本内部配置脚本还没做完。 --------"
sleep 20
